...
class SingleClickAspect {
     ...
     * 方法切入点
     */
        @Pointcut("execution(@com.ygzy.lib_base.aop.SingleClick * *(..))")
    fun method() {}
    ...
    fun aroundJoinPoint(joinPoint: ProceedingJoinPoint, singleClick: SingleClick) {
        ...
        if (currentTimeMillis - lastTime < singleClick.value && (tag == lastTag)) {
            MyLogUtils.d(MyLogUtils.TAG,"SingleClick")
            MyLogUtils.d(MyLogUtils.TAG,String.format("%s 毫秒内发生快速点击：%s", singleClick.value, tag))
            return
        }
